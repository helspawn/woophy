...
if(isset($_POST['submit_delete_comment']) || isset($_POST['submit_delete_all'])){
	...
	if(isset($_POST['submit_delete_all'])){
		$result_pids = DB::query('SELECT photo_id FROM photo_comments WHERE poster_id = '.$uid.';');
		$qry = 'DELETE FROM photo_comments WHERE poster_id = '.$uid.';';
	}else{
		if(isset($_POST['ids']) && is_array($_POST['ids']) && count($_POST['ids'])>0){
			$result_pids = DB::query('SELECT photo_id FROM photo_comments WHERE comment_id IN ('.implode(',', $_POST['ids']).');');
			if(count($_POST['ids']) == $_POST['num']){
				//delete all:
				$qry = 'DELETE FROM photo_comments WHERE poster_id = '.$uid.';';
			}else{
				$qry = 'DELETE FROM photo_comments WHERE comment_id IN ('.implode(',', $_POST['ids']).');';
			}
		}
	}
	if(isset($qry)){
	...
	if($result_pids){
		while($row = DB::fetchAssoc($result_pids)){
			...
			DB::query('UPDATE photos SET comment_count = (SELECT COUNT(*) FROM photo_comments WHERE photo_id = '. $pid .') WHERE photo_id= \''. $pid .'\';');
		}
	}
}
...
<?php
	...
	$limit = 200;
	$uid = (int)$_POST["uid"];
	$query = "SELECT SQL_CALC_FOUND_ROWS * FROM photo_comments WHERE poster_id = $uid ORDER BY comment_id DESC LIMIT 0,$limit;";
	$result = DB::query($query);
	...
	if($result){
		...
		if($num == 0){
			...
			echo '<table><tr><td><b>delete</b></td><td><b>photo</b></td><td><b>comment</b></td><td><b>date</b></td></tr>';
			while($row = DB::fetchAssoc($result)){
				
				echo '<tr>';
				echo '<td><input class="noborder" type="checkbox" name="ids[]" value="'.$row['comment_id'].'"></td>';
				echo '<td><a href="'.ROOT_URL.'photo/'.$row['photo_id'].'" target="_blank">'.$row['photo_id'].'</a></td>';
