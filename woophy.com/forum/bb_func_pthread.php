...
if(strlen($postText)>0 and strlen($postText)<$post_text_minlength) {
...
if($topic_d=db_simpleSelect(0,$Tt,'topic_status,topic_title,posts_count','forum_id','=',$forum,'','','topic_id','=',$topic)) { $lckt=$topic_d[0]; $topicTitle=$topic_d[1]; $totalPosts=$topic_d[2]; } else $lckt=1;
...
$setTpls=array();
$pTxtSm=strip_tags(str_replace(array('<br />','&#039;','&quot;','&amp;','&#036;'), array("\r\n","'",'"','&','$'), $postText));
...
sendMail($v[0], $subS, $msgS, $admin_email, $admin_email);
}//foreach
...
sendMail($admin_email, $subS, $msgS, $admin_email, $admin_email);
}
