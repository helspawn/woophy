...
Latest File Update: 2007-Jan-12
*/
define ('INCLUDED776',1);

$unset=array('logged_admin','isMod','user_id','langu','includeHeader','includeFooter', 'mod_rewrite', 'inss', 'insres');
...
$main_url=rtrim(ABSURL,'/');//changed 20070805 by MG
...
//if($handle=@opendir($GLOBALS['img_url'].'/forum_icons')) {
if($handle=@opendir($GLOBALS['img_path'].DIRECTORY_SEPARATOR.'forum_icons')) {//changed 20070805 by MG
$iconList = '';
$ss=0;
while (($file=readdir($handle))!==false) {
if ($file != '.' && $file != '..' and (substr(strtolower($file),-3)=='gif' OR substr(strtolower($file),-3)=='jpg' OR substr(strtolower($file),-4)=='jpeg')) {
$iconList.="<a href=\"JavaScript:paste_strinL('{$file}')\" onmouseover=\"window.status='{$GLOBALS['l_forumIcon']}: {$file}'; return true\"><img src=\"{$GLOBALS['img_url']}/forum_icons/{$file}\" alt=\"{$file}\" /></a>&nbsp;&nbsp;";
$ss++;
if ($ss==5) {
$ss=0;
$iconList.="<br />\n";
...
if($row[5]!='') $fast.="<img src=\"{$GLOBALS['img_url']}/!pix.gif\" style=\"width:16px;height:16px\" alt=\"\" />&nbsp;<strong>{$row[5]}</strong><br />";

$fast.="<img src=\"{$GLOBALS['img_url']}/forum_icons/{$row[4]}\" style=\"width:16px;height:16px\" alt=\"Forum icon\" />&nbsp;<b><a href=\"{$GLOBALS['main_url']}/{$GLOBALS['bb_admin']}action=editforum2&amp;forumID={$row[0]}\">{$row[1]}</a></b> [ORDER: {$row[3]}] - <i><span class=\"txtSm\">{$row[2]}</span></i>&nbsp;&nbsp; <a href=\"{$GLOBALS['main_url']}/{$GLOBALS['bb_admin']}action=move&amp;where=1&amp;forumID={$row[0]}\">&uarr;</a>&nbsp;&nbsp;<a href=\"{$GLOBALS['main_url']}/{$GLOBALS['bb_admin']}action=move&amp;where=0&amp;forumID={$row[0]}\">&darr;</a><br />";

}
while($row=db_simpleSelect(1));//changed by MG 20070805
}
...
if(!ini_get('magic_quotes_gpc')) $$key=addslashes(trim($val)); else $$key=trim($val);
...
if (file_exists($img_path.DIRECTORY_SEPARATOR.'forum_icons'.DIRECTORY_SEPARATOR."{$forum_icon}")) {
...
if(isset($_GET['page'])) $page=$_GET['page']+0; elseif(isset($_POST['page'])) $page=$_POST['page']+0; else $page=0;
...
if(isset($_POST['anchor']) and $_POST['anchor']!=0) $anchor='#'.($_POST['anchor']+0); else $anchor='';
