...
Latest File Update: 2007-Jan-19
*/
...
updateArray(array($dbUserAct),$Tu,$dbUserId,$user);//CHANGED 20080116 by MG
}
...
if(isset($mod_rewrite) and $mod_rewrite) $urlp="{$lastT[1]}_{$lastT[0]}_0.html"; else $urlp="{$indexphp}action=vthread&amp;topic={$lastT[0]}&amp;forum={$lastT[1]}&amp;page=0";
...
$topicVals[$lastT[0]]="<li><a href=\"{$main_url}/{$indexphp}action=search&amp;loc=1&amp;topic={$lastT[0]}&amp;forum={$lastT[1]}&amp;page={$postsAll[$lastT[0]]}\" rel=\"nofollow\">{$lastT[2]}</a></li>";
}
while ($lastT=db_simpleSelect(1));
}
